    ...
    const getData = async () => {
      try {
        ...
        const res = await axios.get(
          `${backend_url}/users/${user.userId}/deliveries/${task}/delivered/past`,
        );
          ...
          data.map((item, index) => {
            return (
              <View key={index}>
                ...
                <View
                  className={`${isDark ? "bg-[#1F2937]" : "bg-[#F7F7F7]"} min-h-48 mt-3 rounded-3xl p-3`}
                >
                  {item.deliveries?.map((metadata, idx) => {
                    return (
                      <View
                        key={idx}
                        className="flex-row gap-3 items-center mt-1"
                      >
                        <Feather
                          name="map-pin"
                        ...
                        />
                        <Pressable
                          className={`flex-row min-h-12 py-1 justify-between items-center border-b ${isDark ? "border-gray-700" : "border-gray-200"} flex-1`}
                        >
                          <View className="flex-1 pr-2">
                            ...
                            <Text
                              ...
                              style={{ fontFamily: "Inter_400Regular" }}
                            >
                              {truncateString(metadata.pickupAddress, 15)} -{" "}
                              {truncateString(metadata.dropoffAddress, 10)}
                            </Text>
                            <Text
