...
const SendParcels = () => {
  ...
  const task = useLocalSearchParams().task || "send";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push("/(send)/confirmSendRoute");
    } catch (error) {
  ...
  // Pickup Location UI Block (uses mt-10)
  const pickupSection = (
    <View>
  ...
  // Dropoff Location UI Block (updated to use mt-10 to match pickup spacing)
      ...
      <View className={`${headerBg} py-3 pl-5`}>
        <View className="flex-row items-center p-1">
          ...
          <Text
            ...
            style={{ fontFamily: "Inter_600SemiBold" }}
          >
            {task === "send" ? "Send Parcel" : "Receive Parcel"}
          </Text>
        </View>
